...
def merge_second_batch(batch_list):
    ...
    for key, elems in example_merged.items():
        if key in [
                'voxels', 'num_points', 'num_gt', 'voxel_labels',  'gt_names','gt_classes','gt_boxes','gt_importance', 'gt_numbers'
        ]:
...
def prep_pointcloud(input_dict,
    ...
    gt_numb=np.array(gt_dict["gt_boxes"].shape[0])
    example["gt_numbers"]= np.expand_dims(gt_numb, axis=0)
    example.update({
        "gt_boxes": gt_dict["gt_boxes"],
        "gt_classes": gt_dict["gt_classes"],
        "gt_importance": gt_dict["gt_importance"]
    })
