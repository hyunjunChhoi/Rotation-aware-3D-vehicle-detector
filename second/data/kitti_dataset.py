...
def kitti_anno_to_label_file(annos, folder):
    ...
    for anno in annos:
        ...
        for j in range(anno["bbox"].shape[0]):
            kkt=np.expand_dims(anno["dimensions"][j], axis=0)
            kkt2=kkt[:, [1, 2, 0]]
            kkt3=np.squeeze(kkt2, axis=0)
            label_dict = {
                'name': anno["name"][j],
                'alpha': anno["alpha"][j],
                'bbox': anno["bbox"][j],
...
def kitti_anno_to_label_file2(annos_path, folder):
    with open(annos_path, 'rb') as f:
        annos = pickle.load(f)
    dt_annos = convert_detection_to_kitti_annos(annos)

    folder = Path(folder)
    for dt_annos in annos:
        image_idx = anno["metadata"]["image_idx"]
        label_lines = []
        for j in range(anno["bbox"].shape[0]):
            label_dict = {
                ...
                'bbox': anno["bbox"][j],
                'location': anno["location"][j],
                'dimensions': anno["dimensions"][j][:,[1,2,0]],
                'rotation_y': anno["rotation_y"][j],
                'score': anno["score"][j],
            }
            label_line = kitti.kitti_result_line(label_dict)
