...
def get_pointcloud():
    ...
    #sensor_data = BACKEND.dataset.get_sensor_data(idx)
    real_idx=sensor_data["metadata"]["image_idx"]
    print(real_idx)
...
def inference_by_idx():
    ...
    for member in BACKEND.dt_annos:
        #print(member["metadata"]['image_idx'])
        #print(real_idx.type)
        if int(member["metadata"]['image_idx'])==int(real_idx):
            true_member=member
    box3d = true_member["box3d_lidar"].detach().cpu().numpy()
    locs = box3d[:, :3]
    dims = box3d[:, 3:6]
    ...
    response["dt_labels"] = true_member["label_preds"].detach().cpu().numpy().tolist()
    response["dt_scores"] = true_member["scores"].detach().cpu().numpy().tolist()
