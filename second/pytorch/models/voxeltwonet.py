...
class VoxeltwoNet(nn.Module):
    def __init__(self,
                 ...
                 nms_train_first_pre_max_sizes=None,
                 nms_train_first_post_max_sizes = None,
                 nms_train_second_pre_max_sizes = None,
                 nms_train_second_post_max_sizes = None,
                 nms_test_first_pre_max_sizes = None,
                 nms_test_first_post_max_sizes = None,
                 nms_test_second_pre_max_sizes =None,
                 nms_test_second_post_max_sizes =None,
                 nms_first_iou_thresholds =None,
                 nms_second_iou_thresholds =None,
        ...
        ######neww################3
        self._nms_train_first_pre_max_sizes = nms_train_first_pre_max_sizes
        self._nms_train_first_post_max_sizes =nms_train_first_post_max_sizes
        self._nms_train_second_pre_max_sizes = nms_train_second_pre_max_sizes
        self._nms_train_second_post_max_sizes =nms_train_second_post_max_sizes
        self._nms_test_first_pre_max_sizes = nms_test_first_pre_max_sizes
        self._nms_test_first_post_max_sizes = nms_test_first_post_max_sizes
        self._nms_test_second_pre_max_sizes = nms_test_second_pre_max_sizes
        self._nms_test_second_post_max_sizes = nms_test_second_post_max_sizes
        self._nms_first_iou_thresholds =nms_first_iou_thresholds
        self._nms_second_iou_thresholds = nms_second_iou_thresholds
        ##############################################
        ...
        self.rpn_acc2 = metrics2.Accuracy(
            dim=-1, encode_background_as_zeros=encode_background_as_zeros)
        self.rpn_precision2 = metrics2.Precision(dim=-1)
        self.rpn_recall2 = metrics2.Recall(dim=-1)
        self.rpn_metrics2 = metrics2.PrecisionRecall(
            dim=-1,
            thresholds=[0.1, 0.3, 0.5, 0.7, 0.8, 0.9, 0.95],
            use_sigmoid_score=use_sigmoid_score,
            encode_background_as_zeros=encode_background_as_zeros)

        self.rpn_cls_loss2 = metrics2.Scalar()
        self.rpn_loc_loss2 = metrics2.Scalar()
        self.rpn_total_loss2 = metrics2.Scalar()
        ...
        "BG_IOU_THRESHOLD" : 0.65,
        "NMS": 0.01,
        "SCORE_THRESH" :0.01,
        "POOLER_SCALE" : [1.0]*int(self._nms_post_max_sizes[0]), #used as batchsize
        "POOLER_RESOLUTION" : 7, #used as batchsize
        "OUT_CHANNELS": 128 ## 384 for point pillar , 128 for second
        }
        self.ROI_head=box_head.build_roi_box_head(cfg, self.additional_info)
    ...
    def network_forward(self, voxels, num_points, coors, batch_size):
        ...
        preds_dict, feature_final = self.rpn(spatial_features)
        self.end_timer("rpn forward")
        # self.start_timer("roi_head")
        # input : feature_final output: second_prediction

        # self.end_timer("roi_head")
        return preds_dict, feature_final

    def forward(self, example, is_training=False):
        """module's forward should always accept dict and return loss.
        """

        if is_training:
            return self._forward_train(example)
        else:
            return self._forward_test(example)

    def _forward_train(self, example):
        """module's forward should always accept dict and return loss.
        """
        ...
        # coors: [num_voxels, 4]
        preds_dict, feature_final = self.network_forward(voxels, num_points, coors, batch_size_dev)
        # need to check size.
        ...
        ## originally retrun loss from preds_dict and example that is all

        ## now ---> from preds_dict , post processing and make proposal and feed into next ROI-head network

        # preds_dict_copy = copy.deepcopy(preds_dict)
        self.start_timer("predict_train")
        with torch.no_grad():
            res, gtt = self.predict_train(example, preds_dict)
        self.end_timer("predict_train")
        # res= periodic_angle(res)
        for i in range(len(res)):
            res[i]["box3d_lidar"][:, 6] = res[i]["box3d_lidar"][:, 6] - torch.floor(
                res[i]["box3d_lidar"][:, 6] / (2 * math.pi) + 1 / 2) * (2 * math.pi)

        Rboxlists = []
        targetboxlists = []
        # featurelists=[]
        # print(feature_final.size()[0])
        batch_length = feature_final.size()[0]
        # bb=torch.split(feature_final, len(res), dim=0)
        featurelists = torch.chunk(feature_final, batch_length, dim=0)

        # for i in range(batch_length):
        #    featurelists.append(feature_final[i,:].to('cuda:0'))

        for i in range(len(res)):
            rbox_temp = bounding_box.RBoxList(bbox=res[i]["box3d_lidar"], image_size=(200, 176), mode="xywha").to(
                'cuda:0')
            Rboxlists.append(rbox_temp.to('cuda:0'))
            target_temp = bounding_box.RBoxList(bbox=gtt[i]["box3d_lidar"], image_size=(200, 176), mode="xywha")
            target_temp.add_field("labels", gtt[i]["label_preds"])
            # print( gtt[i]["label_preds"])
            targetboxlists.append(target_temp.to('cuda:0'))

        self.start_timer("roi_head_train")
        class_logit, final_prediction, loss_second = self.ROI_head(featurelists, Rboxlists, res, targetboxlists, batch_idx,
                                                                   self._cls_loss_ftor, self._loc_loss_ftor, None,
                                                                   is_training=True)  ##chachange
        self.end_timer("roi_head_train")

        # nms and sampling was done by predict_train
        # function get input res(=proposal) and feature_final and example(groundtruth)
        # x, loss_second, final_prediction=roi_head(feature_final, res, example)
        class_logit_copy = class_logit.detach().clone()
        # final_prediction_copy=final_prediction.detach().clone()
        # final prediction to

        losses = {}
        losses.update(loss_second)

        # finally return final_prediction
        return res, self.loss(example, preds_dict), losses, class_logit_copy, final_prediction

    def _forward_test(self, example):
        """module's forward should always accept dict and return loss.
        """
        batch_size = example['anchors'].shape[0]
        if "metadata" not in example or len(example["metadata"]) == 0:
        ...
        with torch.no_grad():
            preds_dict, feature_final = self.network_forward(voxels, num_points, coors, batch_size_dev)
            self.start_timer("predict_test")

            res, res2, res3 = self.predict_test(example,
                                          preds_dict)  # first stage -> RPN ##originally res is the final result and return it
            self.end_timer("predict_test")
            #print(res[0]["scores"].size())
            for i in range(len(res)):
                res[i]["box3d_lidar"][:, 6] = res[i]["box3d_lidar"][:, 6] - torch.floor(
                    res[i]["box3d_lidar"][:, 6] / (2 * math.pi) + 1 / 2) * (2 * math.pi)
            ...
            for i in range(len(res)):
                Rboxlists.append(
                    bounding_box.RBoxList(bbox=res[i]["box3d_lidar"], image_size=(200, 176), mode="xywha").to('cuda:0'))
                # target_temp=bounding_box.RBoxList(bbox=gtt[i]["box3d_lidar"][:, [0, 1, 3, 4, 6]], image_size=(200, 176), mode="xywha")
                # target_temp.add_field("labels", gtt[i]["label_preds"])
                # targetboxlists.append(target_temp.to('cuda:0'))

            # OI_head=box_head.ROIBoxHead(cfg, self.additional_info)
            # ROI_head=ROI_head.to('cuda:0')
            self.start_timer("roi_head_test")
            x=None
            if list(res[0]["scores"].size())==[0]:
                final_prediction=res
                final_prediction2=res
                final_prediction3=res
                final_prediction4=res
            else:
                x, final_prediction, final_prediction2, final_prediction3, final_prediction4 = self.ROI_head(featurelists, Rboxlists, res, post_max_sizes=self._nms_test_first_post_max_sizes[0],
                                                                                      is_training=False)

            self.end_timer("roi_head_test")
            if x is not None:
                self.time_array.append(x)
            ...
            new_dict = []
            # temp_dict={}
            for i, member in enumerate(final_prediction):
                temp_dict = {}
                temp_dict["box3d_lidar"] = member["box3d_lidar"]
                temp_dict["scores"] = member["scores"]
                temp_dict["label_preds"] = member["label_preds"]
                temp_dict["metadata"] = meta_list[i]
                new_dict.append(temp_dict)

            new_dict_2 = []
            # temp_dict={}
            for i2, member2 in enumerate(final_prediction2):
                temp_dict_2 = {}
                temp_dict_2["box3d_lidar"] = member2["box3d_lidar"]
                temp_dict_2["scores"] = member2["scores"]
                temp_dict_2["label_preds"] = member2["label_preds"]
                temp_dict_2["metadata"] = meta_list[i2]
                new_dict_2.append(temp_dict_2)

            new_dict_3 = []
            # temp_dict={}
            for i3, member3 in enumerate(final_prediction3):
                temp_dict_3 = {}
                temp_dict_3["box3d_lidar"] = member3["box3d_lidar"]
                temp_dict_3["scores"] = member3["scores"]
                temp_dict_3["label_preds"] = member3["label_preds"]
                temp_dict_3["metadata"] = meta_list[i3]
                new_dict_3.append(temp_dict_3)
            new_dict_4 = []
            for i4, member4 in enumerate(final_prediction4):
                temp_dict_4 = {}
                temp_dict_4["box3d_lidar"] = member4["box3d_lidar"]
                temp_dict_4["scores"] = member4["scores"]
                temp_dict_4["label_preds"] = member4["label_preds"]
                temp_dict_4["metadata"] = meta_list[i4]
                new_dict_4.append(temp_dict_4)
            # nms and sampling was done by predict_train\
            # recur proposal is final prediction
            # no gt and just get input res(=proposal) and feature final
            print("thisis res")
            print(res[0]["scores"].size())
            # print(res[1]["scores"].size())
            # print(res[2]["scores"].size())
            # print(res[3]["scores"].size())

            print("thisis final result")
            print(new_dict[0]["scores"].size())
            # print(new_dict[1]["scores"].size())
            # print(new_dict[2]["scores"].size())
            # print(new_dict[3]["scores"].size())

        return res, res2, res3, new_dict, new_dict_2, new_dict_3, new_dict_4
    ...
    def predict(self, example, preds_dict):
        ...
        for box_preds, cls_preds, dir_preds, a_mask, meta in zip(
            ...
            if self._multiclass_nms:
                ...
                iou_thresholds = self._nms_first_iou_thresholds
    ...
    def predict_train(self, example, preds_dict):
        """start with v1.6.0, this function don't contain any kitti-specific code.
        Returns:
            predict: list of pred_dict.
            pred_dict: {
        ...
        batch_anchors = example["anchors"].view(batch_size, -1,
                                                example["anchors"].shape[-1])
        #print( example["gt_boxes"].shape)
        #print(example["gt_classes"].shape)
        #print(example["gt_importance"].shape)
        #print(example["gt_numbers"])
        #print(example["labels"].shape)
        #print(example["importance"].shape)
        batch_gt=[]
        batch_gt_class=[]
        ii=0
        for member in example["gt_numbers"]:
            batch_gt.append(example["gt_boxes"][ii:ii+member])
            batch_gt_class.append(example["gt_classes"][ii:ii+member])
            ii+=member
        #print(batch_gt)
        #batch_gt=example["gt_boxes"].view(batch_size, -1 , example["gt_boxes"].shape[-1])
        #batch_gt_class=example["gt_classes"].view(batch_size,-1,1)
        if "anchors_mask" not in example:
            batch_anchors_mask = [None] * batch_size
        else:
            batch_anchors_mask = example["anchors_mask"].view(batch_size, -1)
        #yy = np.reshape(example["labels"], (batch_size, 6, 200, 176, -1))
        t = time.time()
        batch_box_preds = preds_dict["box_preds"].detach().clone()
        batch_cls_preds = preds_dict["cls_preds"].detach().clone()
        ...
        if self._nms_pre_max_sizes[0]==1000:
            batch_box_preds = self._box_coder.decode_torch(batch_box_preds,
                                                       batch_anchors)
        else:
            batch_box_preds = self._box_coder.decode_torch2(batch_box_preds,
                                                           batch_anchors)

        if self._use_direction_classifier:
            batch_dir_preds = preds_dict["dir_cls_preds"].detach().clone()
            batch_dir_preds = batch_dir_preds.view(batch_size, -1,
                                                   self._num_direction_bins)
        else:
            batch_dir_preds = [None] * batch_size
        ...
        if len(self._post_center_range) > 0:
            post_center_range = torch.tensor(
                ...
                dtype=batch_box_preds.dtype,
                device=batch_box_preds.device).float()
        for box_preds, cls_preds, dir_preds, a_mask, meta, gt_b, gt_c in zip(
                batch_box_preds, batch_cls_preds, batch_dir_preds,
                batch_anchors_mask, meta_list, batch_gt, batch_gt_class):
            if a_mask is not None:
                box_preds = box_preds[a_mask]
                cls_preds = cls_preds[a_mask]
            box_preds = box_preds.float()
            ...
            if self._use_direction_classifier:
                if a_mask is not None:
                    dir_preds = dir_preds[a_mask]
                dir_labels = torch.max(dir_preds, dim=-1)[1]
            if self._encode_background_as_zeros:  ##on
                # this don't support softmax
                assert self._use_sigmoid_score is True
                total_scores = torch.sigmoid(cls_preds)
            else:
            ...
            if self._multiclass_nms:
                ...
                ##chchange####
                score_threshs = self._nms_score_thresholds
                pre_max_sizes = self._nms_train_first_pre_max_sizes
                post_max_sizes = self._nms_train_first_post_max_sizes
                iou_thresholds = self._nms_first_iou_thresholds
                ############################################3
                ...
                if top_scores.shape[0] != 0:  ##on
                    ...
                    selected = nms_func(                 ##on  here sigmoid score to 0~1 normalized scored
                        boxes_for_nms,
                        top_scores,
                        pre_max_size=self._nms_train_first_pre_max_sizes[0],
                        post_max_size=self._nms_train_first_post_max_sizes[0],
                        iou_threshold=self._nms_first_iou_thresholds[0],
                    )
                else:
                    selected = []
                # if selected is not None:
                ...
                selected_scores = top_scores[selected]
            # finally generate predictions.
            if selected_boxes.shape[0] != 0: ##on
                box_preds = selected_boxes
                scores = selected_scores
                label_preds = selected_labels
                if self._use_direction_classifier:
                ...
                final_scores = scores
                final_labels = label_preds
                #print(final_box_preds.size())
                gt_b_tensor=torch.tensor(gt_b,dtype=box_preds.dtype, device=box_preds.device)
                gt_c_tensor=torch.tensor(gt_c,dtype=torch.long, device=box_preds.device)
                gt_score=torch.ones([len(gt_b)], dtype=box_preds.dtype, device=box_preds.device)
                #print(gt_b_tensor.size())
                #yy=torch.reshape(example["labels"],(batch_size,6,200,176,-1))
                #result = torch.where(yy == 1)
                #result=(yy!=0).nonzero()

                ##################################3debbiggugjgjg###############################3
                #compare with the gt tnsor and label ##################################
                #print(gt_c_tensor.size())
                #print(gt_score.size())
                if post_center_range is not None:
                    mask = (final_box_preds[:, :3] >=
                            post_center_range[:3]).all(1)
                    ...
                    #torch.cat([a, b], dim=0)
                    gtt_dict={
                        "box3d_lidar":  gt_b_tensor,
                        "scores": gt_score,
                        "label_preds": gt_c_tensor,
                        "metadata": meta,
                    }
                    predictions_dict = {
                        "box3d_lidar": torch.cat([final_box_preds[mask], gt_b_tensor], dim=0),
                        "scores": torch.cat([final_scores[mask], gt_score], dim=0),
                        "label_preds": torch.cat([label_preds[mask], gt_c_tensor], dim=0),
                        "metadata": meta,
                    }
                    ...
                    predictions_dict = {
                        "box3d_lidar": torch.cat([final_box_preds, gt_b_tensor], dim=0),
                        "scores": torch.cat([final_scores, gt_score], dim=0),
                        "label_preds": torch.cat([label_preds, gt_c_tensor], dim=0),
                        "metadata": meta,
                    }
            else:
                dtype = batch_box_preds.dtype
                device = batch_box_preds.device
                gt_b_tensor=torch.tensor(gt_b,dtype=dtype, device=device)
                gt_c_tensor=torch.tensor(gt_c,dtype=dtype, device=device)
                gt_score=torch.ones([len(gt_b),1], dtype=dtype, device=device)
                predictions_dict = {
                    "box3d_lidar": gt_b_tensor,
                    "scores": gt_score,
                    "label_preds": gt_c_tensor,
        ...
        #    }
            predictions_dicts.append(predictions_dict)
            gtt_dicts.append(gtt_dict)
            #predictions_dicts.append(gt_dict)

            #############################3adding gt proposals to training#######################################
        return predictions_dicts, gtt_dicts

    def predict_test(self, example, preds_dict):
        """start with v1.6.0, this function don't contain any kitti-specific code.
        Returns:
            predict: list of pred_dict.
            pred_dict: {
        ...
        for box_preds, cls_preds, dir_preds, a_mask, meta in zip(
            ...
            if self._multiclass_nms:
                ...
                ##chchange####
                score_threshs = self._nms_score_thresholds
                pre_max_sizes = self._nms_test_first_pre_max_sizes
                post_max_sizes = self._nms_test_first_post_max_sizes
                iou_thresholds = self._nms_first_iou_thresholds
                ############################################3
                ...
                if top_scores.shape[0] != 0:
                    ...
                    selected = nms_func(
                        boxes_for_nms,
                        top_scores,
                        pre_max_size=self._nms_test_first_pre_max_sizes[0],
                        post_max_size=self._nms_test_first_post_max_sizes[0],
                        iou_threshold=1.1,
                    )
                    selected2 = nms_func(
                        boxes_for_nms,
                    ...
                    selected3 = nms_func(
                        boxes_for_nms,
                        top_scores,
                        pre_max_size=self._nms_test_first_post_max_sizes[0],
                        post_max_size=100,
                        iou_threshold=self._nms_iou_thresholds[0],
                    )
                ...
                # if selected is not None:
                selected_boxes = box_preds[selected]
                selected_boxes2 = box_preds[selected2]
                selected_boxes3 = box_preds[selected3]

                if self._use_direction_classifier:
                    selected_dir_labels = dir_labels[selected]
                    selected_dir_labels2 = dir_labels[selected2]
                    selected_dir_labels3 = dir_labels[selected3]
            ...
            if selected_boxes.shape[0] != 0:
                ...
                if self._use_direction_classifier:
                    ...
                    box_preds[
                        ...
                        6] = dir_rot + self._dir_offset + period * dir_labels.to(
                        box_preds.dtype)
                    dir_labels2 = selected_dir_labels2
                    period2 = (2 * np.pi / self._num_direction_bins)
                    dir_rot2 = box_torch_ops.limit_period(
                        box_preds2[..., 6] - self._dir_offset,
                        self._dir_limit_offset, period2)
                    box_preds2[
                        ...,
                        6] = dir_rot2 + self._dir_offset + period2 * dir_labels2.to(
                        box_preds.dtype)

                    dir_labels3 = selected_dir_labels3
                    period3 = (2 * np.pi / self._num_direction_bins)
                    dir_rot3 = box_torch_ops.limit_period(
                        box_preds3[..., 6] - self._dir_offset,
                        self._dir_limit_offset, period3)
                    box_preds3[
                        ...,
                        6] = dir_rot3 + self._dir_offset + period3 * dir_labels3.to(
                        box_preds.dtype)
                final_box_preds = box_preds
                final_scores = scores
                final_labels = label_preds
                final_box_preds2 = box_preds2
                final_scores2 = scores2
                final_labels2 = label_preds2
                final_box_preds3 = box_preds3
                ...
                if post_center_range is not None:
                    ...
                    mask2 = (final_box_preds2[:, :3] >=
                             post_center_range[:3]).all(1)
                    mask2 &= (final_box_preds2[:, :3] <=
                              post_center_range[3:]).all(1)
                    predictions_dict2 = {
                        "box3d_lidar": final_box_preds2[mask2],
                        "scores": final_scores2[mask2],
                        "label_preds": label_preds2[mask2],
                        "metadata": meta,
                    }

                    mask3 = (final_box_preds3[:, :3] >=
                             post_center_range[:3]).all(1)
                    mask3 &= (final_box_preds3[:, :3] <=
                              post_center_range[3:]).all(1)
                    predictions_dict3 = {
                        "box3d_lidar": final_box_preds3[mask3],
                        "scores": final_scores3[mask3],
                        "label_preds": label_preds3[mask3],
                        "metadata": meta,
                    }
                else:
                    predictions_dict = {
            ...
            #############################no need to add gt proposals#######################################

        return predictions_dicts, predictions_dicts2, predictions_dicts3
    ...
    def update_metrics(self, cls_loss, loc_loss, cls_preds, labels, sampled):
        ...
        #print("cls resized original size")
        ...
        return ret
    def update_metrics2(self, cls_loss, loc_loss, cls_preds, labels):
        batch_size = 1
        num_class = self._num_class
        if not self._encode_background_as_zeros:
            num_class += 1
        cls_preds = cls_preds.view(batch_size, -1, num_class)
        #print("cls_reiszed size")
        #print(cls_preds.size())
        rpn_acc = self.rpn_acc2(labels, cls_preds).numpy()[0]
        prec, recall = self.rpn_metrics2(labels, cls_preds)
        prec = prec.numpy()
        recall = recall.numpy()
        rpn_cls_loss = self.rpn_cls_loss2(cls_loss).numpy()[0]
        rpn_loc_loss = self.rpn_loc_loss2(loc_loss).numpy()[0]
        ret = {
            "loss_two": {
                "cls_loss_two": float(rpn_cls_loss),
                "cls_loss_rt_two": float(cls_loss.data.cpu().numpy()),
                'loc_loss_two': float(rpn_loc_loss),
                "loc_loss_rt_two": float(loc_loss.data.cpu().numpy()),
            },
            "rpn_acc_two": float(rpn_acc),
            "pr_two": {},
        }
        for i, thresh in enumerate(self.rpn_metrics.thresholds):
            ret["pr_two"][f"prec@{int(thresh*100)}"] = float(prec[i])
            ret["pr_two"][f"rec@{int(thresh*100)}"] = float(recall[i])
        return ret
    ...
    def clear_metrics2(self):
        self.rpn_acc2.clear()
        self.rpn_metrics2.clear()
        self.rpn_cls_loss2.clear()
        self.rpn_loc_loss2.clear()
        self.rpn_total_loss2.clear()
    @staticmethod
    def convert_norm_to_float(net):
        ...
        for child in net.children():
            VoxeltwoNet.convert_norm_to_float(child)
        return net
...
def create_loss(loc_loss_ftor,
    ...
    if encode_rad_error_by_sin:
        ...
        box_preds, reg_targets = add_sin_difference(box_preds, reg_targets, box_preds[..., 6:7], reg_targets[..., 6:7],
                                                    sin_error_factor)
    #print("this is one hot_targets")
    #print(one_hot_targets.size(), one_hot_targets)

    #print("this is cls_preds")
    #print(cls_preds.size() , cls_preds)
    #print("this is box preds")
    #print(box_preds.size() , box_preds)
    #print("this is reg targets")
    #print(reg_targets.size(), reg_targets)
...
def prepare_loss_weights(labels,
    ...
    if loss_norm_type == LossNormType.NormByNumExamples:
        ...
        neg_normalizer = negatives.sum(1, keepdim=True).type(dtype)
