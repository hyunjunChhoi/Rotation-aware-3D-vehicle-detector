...
from second.pytorch.models.voxeltwonet import LossNormType, get_voxelnet_class
def build(model_cfg: second_pb2.VoxelNet, voxel_generator,
    ...
    if len(model_cfg.target_assigner.nms_train_first_pre_max_sizes) != 0:
        nms_train_first_pre_max_sizes = list(model_cfg.target_assigner.nms_train_first_pre_max_sizes)
        assert len(nms_train_first_pre_max_sizes) == num_class
    else:
        nms_train_first_pre_max_sizes = [c.nms_train_first_pre_max_size for c in classes_cfg]

    if len(model_cfg.target_assigner.nms_train_first_post_max_sizes) != 0:
        nms_train_first_post_max_sizes = list(model_cfg.target_assigner.nms_train_first_post_max_sizes)
        assert len(nms_train_first_post_max_sizes) == num_class
    else:
        nms_train_first_post_max_sizes = [c.nms_train_first_post_max_size for c in classes_cfg]

    if len(model_cfg.target_assigner.nms_train_second_pre_max_sizes) != 0:
        nms_train_second_pre_max_sizes = list(model_cfg.target_assigner.nms_train_second_pre_max_sizes)
        assert len(nms_train_second_pre_max_sizes) == num_class
    else:
        nms_train_second_pre_max_sizes = [c.nms_train_second_pre_max_size for c in classes_cfg]

    if len(model_cfg.target_assigner.nms_train_second_post_max_sizes) != 0:
        nms_train_second_post_max_sizes = list(model_cfg.target_assigner.nms_train_second_post_max_sizes)
        assert len(nms_train_second_post_max_sizes) == num_class
    else:
        nms_train_second_post_max_sizes = [c.nms_train_second_post_max_size for c in classes_cfg]

    if len(model_cfg.target_assigner.nms_test_first_pre_max_sizes) != 0:
        nms_test_first_pre_max_sizes = list(model_cfg.target_assigner.nms_test_first_pre_max_sizes)
        assert len(nms_test_first_pre_max_sizes) == num_class
    else:
        nms_test_first_pre_max_sizes = [c.nms_test_first_pre_max_size for c in classes_cfg]

    if len(model_cfg.target_assigner.nms_test_first_post_max_sizes) != 0:
        nms_test_first_post_max_sizes = list(model_cfg.target_assigner.nms_test_first_post_max_sizes)
        assert len(nms_test_first_post_max_sizes) == num_class
    else:
        nms_test_first_post_max_sizes = [c.nms_test_first_post_max_size for c in classes_cfg]

    if len(model_cfg.target_assigner.nms_test_second_pre_max_sizes) != 0:
        nms_test_second_pre_max_sizes = list(model_cfg.target_assigner.nms_test_second_pre_max_sizes)
        assert len(nms_test_second_pre_max_sizes) == num_class
    else:
        nms_test_second_pre_max_sizes = [c.nms_test_second_pre_max_size for c in classes_cfg]


    if len(model_cfg.target_assigner.nms_test_second_post_max_sizes) != 0:
        nms_test_second_post_max_sizes = list(model_cfg.target_assigner.nms_test_second_post_max_sizes)
        assert len(nms_test_second_post_max_sizes) == num_class
    else:
        nms_test_second_post_max_sizes = [c.nms_test_second_post_max_size for c in classes_cfg]


    if len(model_cfg.target_assigner.nms_first_iou_thresholds) != 0:
        nms_first_iou_thresholds = list(model_cfg.target_assigner.nms_first_iou_thresholds)
        assert len(nms_first_iou_thresholds) == num_class
    else:
        nms_first_iou_thresholds = [c.nms_first_iou_threshold for c in classes_cfg]

    if len(model_cfg.target_assigner.nms_second_iou_thresholds) != 0:
        nms_second_iou_thresholds = list(model_cfg.target_assigner.nms_second_iou_thresholds)
        assert len(nms_second_iou_thresholds) == num_class
    else:
        nms_second_iou_thresholds = [c.nms_second_iou_threshold for c in classes_cfg]
    ...
    net = get_voxelnet_class(model_cfg.network_class_name)(
        ...
        nms_train_first_pre_max_sizes=nms_train_first_pre_max_sizes,
        nms_train_first_post_max_sizes=nms_train_first_post_max_sizes,
        nms_train_second_pre_max_sizes=nms_train_second_pre_max_sizes,
        nms_train_second_post_max_sizes=nms_train_second_post_max_sizes,
        nms_test_first_pre_max_sizes=nms_test_first_pre_max_sizes,
        nms_test_first_post_max_sizes=nms_test_first_post_max_sizes,
        nms_test_second_pre_max_sizes=nms_test_second_pre_max_sizes,
        nms_test_second_post_max_sizes=nms_test_second_post_max_sizes,
        nms_first_iou_thresholds=nms_first_iou_thresholds,
        nms_second_iou_thresholds=nms_second_iou_thresholds,
