...
def boxlist_nms(boxlist, nms_thresh, max_proposals=-1, score_field="score", GPU_ID=0):
    ...
    # x,y,h,w,a
    ch_props = ch_proposals[:, [0,1,3,2,6]]	
    # print('ch_proposals:',ch_proposals.shape)
    # print('score_np:', score_np.shape)

    if ch_props.shape[0] < 1:
        return boxlist

    keep = rotate_gpu_nms(np.array(np.hstack((ch_props, score_np[..., np.newaxis])), np.float32), nms_thresh, GPU_ID)  # D
    # print time.time() - tic
    if max_proposals > 0:
...
def remove_small_boxes(boxlist, min_size):
    ...
    _, _, _, ws, hs ,_ ,a_s = xywha_boxes.unbind(dim=1)
    keep = (
...
def boxlist_iou(boxlist1, boxlist2, GPU_ID=0):
    ...
    ch_box11 = ch_box1[:, [0,1,3,2,6]]
    ch_box2 = box2_np.copy()
    ch_box22 = ch_box2[:, [0,1,3,2,6]]
