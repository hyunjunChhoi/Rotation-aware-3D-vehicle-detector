...
class RBoxList(object):
    ...
    def rescale(self, boxes_margin, *args, **kwargs):
        ...
        # ratio = ratios[0]
        scaled_xc, scaled_yc, scaled_zc, scaled_w, scaled_l, scaled_h, scaled_a = self._split_into_xywha()
        # scaled_xc *= ratio
        # scaled_yc *= ratio
        scaled_w *= boxes_margin
        scaled_l *= boxes_margin
        # scaled_angle = a
        scaled_box = torch.cat(
            (scaled_xc, scaled_yc,scaled_zc, scaled_w, scaled_l,  scaled_h, scaled_a), dim=-1
        )
    ...
    def _split_into_xywha(self):
        ...
        if self.mode == 'xywha':
            TO_REMOVE = 1
            xc, yc, zc, w, l, h, a = self.bbox.split(1, dim=-1)
            return (
                xc, yc, zc, w, l, h, a
            )
        else:
